...
| 15 leads Core / semaine | Partiel | Provider OpenAI web a prouvé 15 candidats scannés en smoke sur anciens artefacts ; les payloads agent_tasks sont transmis au worker, mais il manque une preuve courante hebdo Supabase persistée |
| 100 comptes Exploration scannés | Partiel | Provider OpenAI web a prouvé 100 comptes scannés en smoke sur anciens artefacts ; les payloads agent_tasks sont transmis au worker, mais il manque une preuve courante hebdo Supabase persistée |
...
- P0.4 Do-not-contact : gate déterministe ajouté côté TS, worker offline, provider réel avant génération d'outreach et DB ; l'action serveur évite les doublons et la migration récente ajoute des index uniques par scope DNC.
...
- P0.2 Workflows Core/Exploration réels : les volumes PRD ont été prouvés côté provider OpenAI web sur artefacts anciens, mais pas encore comme runs persistés Supabase sur la révision courante.
