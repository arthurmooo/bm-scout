...
| Feedback loop influente | Partiel | Effet scoring/message/blocage/angle prouvé par tests locaux TS + worker provider et compteurs `feedback_memory_effects` ; actions feedback/outcome écrivent la mémoire Supabase ; pas encore prouvé par run réel Supabase à volume |
...
- P0.5 Feedback loop : mémoire locale causale ajoutée côté TS et worker provider pour rejet, pénalité secteur, bonus angle, anti-générique et DNC ; le worker lit maintenant feedbacks, outcomes et DNC Supabase et agrège les impacts feedback dans les run steps.
...
- P0.5 Feedback loop réelle : impact causal prouvé localement dans TS et le worker Python, avec garde-fou readiness sur `feedback_memory_effects`, à prouver avec feedbacks/outcomes Supabase réels à volume.
