...
- `npm run verify:supabase` vérifie maintenant aussi `scout_agent_tasks`, `scout_feedback`, `scout_outcomes`, `scout_do_not_contact`, `scout_run_steps`, `scout_action_events`, au moins une action liée à une tâche par `task_id`, et la fusion RPC domain/Core.
...
- Supabase interne : les nouvelles migrations `scout_messages_no_approved_state` et `scout_dnc_scope_uniqueness` sont versionnées et testées statiquement, mais non appliquées via MCP dans cette session car le connecteur Supabase répond `token_expired`. Le CLI confirme que le projet interne est joignable et qu'aucun message `approved` n'existe actuellement ; l'application du DDL distant reste à refaire après reconnexion MCP ou via pipeline de migration.
...
3. Appliquer les migrations DNC/no-auto-send récentes sur le projet Supabase interne après reconnexion MCP ou via pipeline contrôlé.
4. Prouver les volumes PRD 15 Core / 100 Exploration en run Agents SDK persisté Supabase sur la révision courante, pas seulement via anciens artefacts ou smoke provider.
5. Exécuter `npm run feedback:evidence` avec secrets serveur pour produire une preuve Supabase de causalité feedback, puis prouver la même mémoire sur runs marché Core/Exploration à volume.
6. Exécuter le workflow GitHub Actions `BM Scout readiness evidence` avec secrets, télécharger `bm-scout-readiness-evidence` et vérifier que `quality:readiness` est le gate final.
7. Affecter les claims Supabase réels aux comptes Romu/Arthur et valider le parcours magic link sur le projet interne.
