...
- Actions message auditables : copie et marquage utilisé ciblent les derniers `scout_messages.id` validés par canal, jamais tous les messages historiques d'une company ; les traces `scout_action_events` portent `message_id`, canal et IDs utilisés manuellement sans journaliser le corps complet du message.
