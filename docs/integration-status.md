...
- Run steps feedback worker : chaque application de mémoire produit un step `apply_feedback_memory`, les DNC pré-génération produisent `dnc_pre_generation_gate`, puis un agrégat `feedback_memory_effects` compte les impacts score, blocage, DNC, message régénéré, angle renforcé et delta segment.
