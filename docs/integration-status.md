...
- Supabase interne : migrations `scout_messages_no_approved_state` et `scout_dnc_scope_uniqueness` appliquées via Supabase CLI sur `urlggighkvdcyzsxxzcc` après dry-run `--include-all`. Vérification CLI : historique local/remote aligné jusqu'à `20260531125826`, aucun message `approved`, `137` messages bloqués, `3` entrées DNC.
