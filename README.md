...
- Actions UI branchées sur une API serveur : valider, surveiller, rejeter, exclure, enrichir, relancer QC, copier, DNC, outcomes, raisons feedback en 1 clic, lancer routines. Les copies ne sont écrites dans le presse-papiers qu'après validation serveur, avec le texte exact renvoyé par le serveur.
- Les actions message ne mutent que les `scout_messages.id` explicitement validés par le serveur ; `scout_action_events` trace le `message_id`, le `task_id` des routines lancées, le canal et les IDs utilisés manuellement sans persister le corps complet du message.
...
- DNC hard gate côté qualité TS, côté worker offline, côté provider réel avant génération d'outreach et côté DB pour empêcher un message non bloqué sur une cible DNC ou un outcome négatif ; une action DNC Romu persiste company + domaine + contacts/email hash disponibles, et un DNC ajouté/importé bloque aussi les messages existants liés à l'email, au contact, au domaine ou à l'entreprise.
...
En Core, le gate exige un artefact worker opérationnel avec `final_decision=ready`. En Exploration, `final_decision=not_ready` reste acceptable si le run produit une shortlist actionnable : le PRD interdit de transformer l'exploration en outreach direct sans signal fort.
...
`verify:supabase` écrit aussi `artifacts/supabase-runtime/latest-verify.json`. `quality:readiness` peut utiliser cet artefact si l'env Supabase serveur n'est pas présente au moment du gate, mais uniquement si l'artefact est `pass`, porte la révision courante, contient des compteurs runtime complets, prouve au moins une action Romu reliée à `scout_agent_tasks.task_id`, prouve la fusion RPC par domaine avec priorité Core + cleanup à zéro, prouve par probes temporaires que `approved` est refusé et que les doublons DNC sont bloqués par scope, et n'a pas été produit par un worktree `-dirty`.
